...
blackstonemap <- function(...){
  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        BRCVAR_UI('brcvar', data_db, data_score_db)
  ...
  server <- function(input, output, session) {
    # Add module servers ----
    brcvar <- BRCVAR_SERVER('brcvar', sites_db, parameters_db, data_db,
                            data_score_db, reactive({ input$tabset }))
